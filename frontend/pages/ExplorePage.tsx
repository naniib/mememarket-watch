  ...
  <div className="p-6 space-y-4">
    ...
    <ol className="space-y-3">
      {polishedGems.map(gem => (
        <li key={gem.rank} className="bg-[#161B22] border border-[#30363D] rounded-lg p-4 flex items-center justify-between transition-all hover:border-emerald-400/50 hover:bg-[#1D2127]">
  ...
  <div className="p-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {upcomingLaunches.map(launch => (
        <div key={launch.name} className="border border-emerald-400/50 rounded-lg p-4 bg-[#161B22] flex flex-col items-center space-y-3 transition-all hover:bg-emerald-400/10 hover:shadow-lg hover:shadow-emerald-400/20">
          ...
          <div className="w-full">
            ...
            <div className="w-full bg-gray-700 rounded-full h-2.5">
              <div className="bg-emerald-400 h-2.5 rounded-full" style={{ width: `${launch.buzz}%` }}></div>
            </div>
          </div>
        </div>
      ))}
    ...
    <div className="mt-6">
       <button className="w-full bg-emerald-400 text-black font-bold py-3 px-4 rounded-lg transition-transform hover:scale-105">
...
const TopHunters = () => {
    const getPodiumClass = (rank: number) => {
        switch(rank) {
            ...
            default: return 'border-[#30363D] hover:border-emerald-400/50';
        }
    };
                        ...
                        className={`px-3 py-4 text-sm font-medium transition-all duration-200
                          ${
                            activeTab === tab
                              ...
                              : 'text-gray-400 border-b-2 border-transparent hover:text-emerald-400/80 hover:border-emerald-400/50'
                          }
                        `}
