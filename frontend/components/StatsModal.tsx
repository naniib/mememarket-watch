...
const StatsModal = ({ isOpen, onClose, advancedStats }: StatsModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fade-in">
            <div 
                className="bg-gradient-to-br from-[#10141a] to-[#0D1117] border border-neon-green/30 rounded-2xl shadow-2xl shadow-neon-green/10 w-full max-w-3xl max-h-[90vh] flex flex-col"
            >
                ...
                <footer className="p-6 border-t border-gray-700/50 flex-shrink-0 text-center">
                    <button 
                        onClick={onClose} 
                        className="bg-neon-green text-black font-bold py-2.5 px-8 rounded-lg text-base transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg hover:shadow-neon-green/30"
                    >
                        Cerrar
                    </button>
                </footer>
